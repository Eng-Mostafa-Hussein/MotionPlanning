...
func makeCompactBoxPlot(title, yLabel, key, format string, experiments map[string][]*rdb.Dataset, width, height float64) {
	...
	for mapType, experiment := range experiments {
		...
		var gustAlgSet *rdb.Dataset
		//var gustold1AlgSet *rdb.Dataset		
		//var gustoldAlgSet *rdb.Dataset
		...
		for _, ds := range experiment {
			...
			if ds.GetName() == "Gust" {
				...
				pprmAlgSet = ds
			//} else if ds.GetName() == "Gust_0reg" {
			//	gustoldAlgSet = ds
			//} else if ds.GetName() == "Gust_3reg" {
			//	gustold1AlgSet = ds
			} else if ds.GetName() == "BEAST" {
				beastAlgSet = ds
		...
		fmt.Println(gustAlgSet.GetSize())
		//fmt.Println(gustold1AlgSet.GetSize())
		//fmt.Println(gustoldAlgSet.GetSize())
